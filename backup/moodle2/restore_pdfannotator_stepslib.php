...
class restore_pdfannotator_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function process_pdfannotator_annotation($data) {
        ...
        if (empty($mappeduserid)) {
            $mappeduserid = $this->task->get_userid();
        }
        $data->userid = $mappeduserid;
        if (property_exists($data, 'modifiedby') && !empty($data->modifiedby)) {
            $mappedmodifiedby = $this->get_mappingid('user', $data->modifiedby);
            if (empty($mappedmodifiedby)) {
                $mappedmodifiedby = $this->task->get_userid();
            }
            $data->modifiedby = $mappedmodifiedby;
        }
    ...
    protected function process_pdfannotator_comment($data) {
        ...
        $parentpdfannotatorid = $this->get_new_parentid('pdfannotator');
        if (!empty($parentpdfannotatorid)) {
            $data->pdfannotatorid = $parentpdfannotatorid;
        } else {
            $mappedpdfannotatorid = $this->get_mappingid('pdfannotator', $data->pdfannotatorid);
            if (!empty($mappedpdfannotatorid)) {
                $data->pdfannotatorid = $mappedpdfannotatorid;
            }
        }
