(function () {
    ...
    function scheduleAnnotationRecovery(reason) {
        ...
        state.recoveryTimer = setTimeout(function () {
            ...
            var mustRebuild = !hasPages || likelyHiddenRender || reason === 'shown.bs.tab' || reason === 'pageshow' || reason === 'focus';

            // On return from other tabs/views, force full rebuild to restore PDF + overlays reliably.
            if (mustRebuild) {
                renderDocument();
                return;
            }
    ...
    function bindVisibilityRecovery() {
        ...
        window.addEventListener('pageshow', function () {
            scheduleAnnotationRecovery('pageshow');
        });

        window.addEventListener('focus', function () {
            scheduleAnnotationRecovery('focus');
        });
    ...
    function bootstrap(cm, documentObject, contextId, userId, capabilities, toolbarSettings, page, annoid, commid, editorSettings) {
        ...
        bindToolbar();
        debugLog('boot', 'bootstrap', {bundle: 'v00050'});
        bindViewerScroll();
