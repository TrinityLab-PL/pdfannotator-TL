(function () {
    ...
    function clearViewer() {
        ...
        if (state.annotationWarmupTimer) {
            clearInterval(state.annotationWarmupTimer);
    ...
    function startAnnotationWarmup() {
        ...
        state.annotationWarmupRounds = 0;
        state.annotationWarmupTimer = setInterval(function () {
            if (!state.pdf) {
                clearInterval(state.annotationWarmupTimer);
                state.annotationWarmupTimer = null;
                return;
            }
            state.annotationWarmupRounds += 1;
            for (var pageNo = 1; pageNo <= state.pdf.numPages; pageNo++) {
                loadAndRenderAnnotations(pageNo, true);
            }
            if (state.annotationsLoadedOnce || state.annotationWarmupRounds >= 90) {
                clearInterval(state.annotationWarmupTimer);
                state.annotationWarmupTimer = null;
            }
        }, 1500);
    ...
    function ensureRestoreControls() {
        ...
        var existing = nav.querySelector('.tl-restore-wrap');
        if (existing) {
            return;
        }
